...
class ProcessTrainData(FlowSpec):
    ...
    def preprocess_csv(self):
        ...
        # Clean data
        self.train = clean_traincsv_data(train, pq_train_dirpath=self.data_dir / 'series_train.parquet')
    ...
    def autoencode_pq(self):
        ...
        """
        self.train_agg_encoded, self.autoencoder, self.agg_mean, self.agg_std = autoencode(self.train_agg)
        print(f"Train pq aggregate shape (after autoencoding): {self.train_agg_encoded.shape}")
    ...
    def join_csv_and_pq(self, inputs):
        ...
        self.train = {
            FeatureEngineeringSet.Normal.name: merge_csv_pqagg_data(self.train, self.train_agg),
            FeatureEngineeringSet.AutoencodedPQ.name: merge_csv_pqagg_data(self.train, self.train_agg_encoded)
        }
    ...
    def feature_engineering(self):
        ...
        print(f"[{self.input}] Train shape (after joining): {self.train[self.input].shape}")
        self.df, self.artifacts = feature_engineering(self.train[self.input], training=True)
