        ...
        for paraminfo in params:
            ...
            if paraminfo["model_class"] == "EnsembleModel":
                estimators.append(
                    (
                        paraminfo["name"],
                        EnsembleModel(**paraminfo["params"]).get_booster(),
                    )
                )
            if paraminfo["model_class"] == "XGBRegressor":
                estimators.append(
                    (paraminfo["name"], xgb.XGBRegressor(**paraminfo["params"]))
                )
            elif paraminfo["model_class"] == "LGBMRegressor":
                estimators.append(
                    (
                        paraminfo["name"],
                        lgb.LGBMRegressor(**paraminfo["params"]),
                    )
                )
                ...
                estimators.append(
                    (
                        paraminfo["name"],
                        RandomForestRegressor(**paraminfo["params"]),
                    )
                )
                ...
                estimators.append(
                    (
                        paraminfo["name"],
                        CatBoostRegressor(**paraminfo["params"]),
                    )
                )
