...
def clean_traincsv_data(df, pq_train_dirpath):
    df = (
        df.pipe(handle_zero_weight_bmi)
...
def clean_testcsv_data(df):
    df = df.pipe(handle_zero_weight_bmi)
    return df
...
def autoencode(
    ...
    df_scaled = df_scaled.with_columns(
        [
            (pl.col(c) - agg_mean[c]) / (agg_std[c] + 1e-10)
    ...
    df_transformed = pl.DataFrame(df_transformed.detach().numpy()).with_columns(
        df["id"]
    )
