...
def __add_fgc_bia_tvalues(df: pl.DataFrame, training: bool = False, artifacts: dict[str, Any] = {}) -> pl.DataFrame:
    """
    ...
    def make_meanstd_cols(df: pl.DataFrame, training: bool = False, meanstd_values: pl.DataFrame | None = None):
        if training:
            return df.with_columns(
            ...
            assert meanstd_values is not None, "Artifacts must contain 'meanstd_values' when not training."
            return df.join(
                meanstd_values,
    ...
    df = (
        df
        .pipe(make_new_wh)
        .pipe(make_meanstd_cols, training=training, meanstd_values=artifacts.get('meanstd_values'))
        .pipe(make_tvalues)
    )
...
def __transform_features(df: pl.DataFrame, training: bool = False, artifacts: dict[str, Any] = {}) -> pl.DataFrame:
    """
    ...
    encoded_season_df = pl.DataFrame(res, schema=list(artifacts['encoder'].get_feature_names_out()), orient="row")
...
def feature_engineering(df: pl.DataFrame, training: bool = False, artifacts: dict[str, Any] = {}):
    """
...
def get_features(df: pl.DataFrame) -> list[str]:
    ...
    must_exclude_features = [
        'PAQ_A-Season', 'PAQ_C-PAQ_C_Total', 'PAQ_A-PAQ_A_Total',
    ...
    letstry_exclude_features = [
        'Physical-BMI',
        'BIA-BIA_FFM', 'tBIA-BIA_FFM', 'tBIA-BIA_TBW', 'FGC-FGC_TL_Zone', 'FGC-FGC_SRR_Zone',
        'BIA-Season', 'Basic_Demos-Enroll_Season', 'CGAS-Season', 'SDS-Season', 'FGC-Season',
        'daily_avg_enmo_std', 'light_0.25', 'enmo_0.5', 'rolling_std_anglez_0.25', 'enmo_0.75',
        'non-wear_flag_mean', 'daily_avg_enmo_max', 'light_mean', 'light_0.75', 'daily_avg_light_mean',
        'daily_avg_light_max', 'enmo_std', 'Y_0.5', 'enmo_mean', 'anglez_0.5', 'daily_avg_light_std',
        'X_0.25', 'anglez_0.75', 'rolling_std_anglez_std', 'anglez_0.25', 'X_0.5',
        'rolling_std_anglez_0.5', 'light_0.5', 'daily_avg_light_min', 'rolling_std_anglez_0.75'
    ]
