...
def aggregate_pq_files(files, cols):
    ...
    dfs = []
    curr_total = 0
    for pqfile in track(files, description="Aggregating pq files.."):
...
def relative_days(df):
    return df.with_columns(
        ...
        (pl.col("relative_date_PCIAT") * 86400 + pl.col("time_of_day") / 1e9)
        .diff()
        .fill_null(5)
        .alias("time_diff"),
...
def aggregate_pq_files_v2(files):
    ...
    def _agg_dfs(dfs):
        ...
        first7days_avg_df = (
            df.filter((pl.col("day") - pl.col("day").min()) < 7)
            .group_by(["id", "relative_date_PCIAT"])
            .agg(
                pl.col("enmo").mean().alias("daily_avg_enmo"),
                pl.col("light").mean().alias("daily_avg_light"),
            )
        ...
        result_df = df.group_by("id").agg(
            ...
            (
                pl.col("relative_date_PCIAT").max()
                - pl.col("relative_date_PCIAT").min()
            ).alias("total_days"),
...
def load_time_series(ids) -> pl.DataFrame:
    with ThreadPoolExecutor() as executor:
        results = list(
            track(executor.map(lambda fname: process_file(fname), ids), total=len(ids))
        )

    stats, indexes = zip(*results)
    df = pl.DataFrame(
        stats, schema=[f"stat_{i}" for i in range(len(stats[0]))], orient="row"
    )
